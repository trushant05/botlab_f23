...
// Added variables for speed limitation
static float max_fwd_speed = 10;
static float max_turn_speed = 10* M_PI / 4;
static float min_fwd_speed = -10;
static float min_turn_speed = -10* M_PI / 4;
...
public:
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        ...
        fwd_vel = std::min(std::max(fwd_vel, min_fwd_speed), max_fwd_speed);
        turn_vel = std::min(std::max(turn_vel, min_turn_speed), max_turn_speed);
...
public: 
    ...
    */
    ...
    {
        std::cout << "Entered MotionController Constructor \n";
        subscribeToLcm();
...
private:
    ...
    void subscribeToLcm()
    {
        std::cout << "Entered subscribeToLcm \n";
